...
if str(args.platform) == 'illumina':
	...
	for i in range(1,int(args.training_examples)+1):
		subprocess.call(["art_illumina", "-ss", "MSv3", "-i", str(args.target), "-o", "_tempdir/simulatedTarget-"+str(i), "-f", str(random.uniform(0.005,0.05)), "-l", str(args.read_length)], stdout=FNULL, stderr=FNULL)
